...
function Verif() {
  ...
  if (currentPageURL.indexOf("gestion-etudiant") !== -1) {
    ...
    var boutonPromotionsConcernees = document.querySelector(
      "#promotions-concernees-offre"
    );
    boutonPromotionsConcernees.addEventListener(
      "click",
      TogglePopUpPromotionsCheckbox
    );
    ...
    var boutonSecteurActivite = document.querySelector(
      "#secteur-activite-offre"
    );
    boutonSecteurActivite.addEventListener(
      "click",
      TogglePopUpSecteursActiviteCheckbox
    );
  } else {
    VerifEntreprise();
...
function ajouterIconePopUp(element, valide) {
  var classesSucces = [
    "fa-check-circle",
    "validation-icone-label",
    "validation-icone-popup",
  ];
  var classesErreur = [
    "fa-times-circle",
    "no-validation-icone-label",
    "no-validation-icone-popup",
  ];
  var classesAjout = valide ? classesSucces : classesErreur;
  var classesSuppression = valide ? classesErreur : classesSucces;
...
function VerifEtudiant() {
  ...
  var DateNaissanceEtudiant = document.querySelector(
    "#date-naissance-etudiant"
  );
  var ValidationDateNaissance = VerifDateNaissanceEtudiant(
    DateNaissanceEtudiant.value
  );
  ...
  var RegionEtudiant = document.querySelector(
    "#administrative_area_level_1-etudiant"
  );
...
function VerifTuteur() {
  ...
  var ValidationDateNaissance = VerifDateNaissanceTuteur(
    DateNaissanceTuteur.value
  );
  ...
  var RegionTuteur = document.querySelector(
    "#administrative_area_level_1-tuteur"
  );
...
function VerifOffre() {
  ...
  var PromotionsConcernees = document.querySelector(
    "#promotions-concernees-offre"
  );
  ...
  var RegionOffre = document.querySelector(
    "#administrative_area_level_1-offre"
  );
...
function VerifCommentaireEntreprise(valeur, radioChecked) {
  if (
    valeur.trim().length >= 30 &&
    valeur.trim().length <= 1500 &&
    radioChecked
  ) {
    return true;
...
function VerifEntreprise() {
  ...
  var secteurActiviteEntreprise = document.querySelector(
    "#secteur-activite-entreprise"
  );
  ...
  btnResetNote.addEventListener("click", function () {
    radios.forEach(function (radio) {
      radio.checked = false;
      var iconeCommentaire = document.querySelector(
        "#section-commentaire-entreprise i"
      );
      if (iconeCommentaire) {
  ...
  adressesCPEntreprise.forEach((adresse) => {
    ...
    if (adresseInput) {
      ajouterIconeInput(
        adresseInput,
        VerifAdresseEntreprise(adresseInput.value)
      );
    ...
    if (codePostalInput) {
      ajouterIconeInput(
        codePostalInput,
        VerifCodePostalEntreprise(codePostalInput.value)
      );
    }
  });

  var villesRegionsEntreprise = document.querySelectorAll(
  ...
  var descriptionValide = VerifDescriptionEntreprise(
    descriptionEntreprise.value
  );
  ajouterIconeLabel(descriptionEntreprise, descriptionValide);

  var commentaireValide = VerifCommentaireEntreprise(
    commentaireEntreprise.value,
    radioChecked
  );
